"""Tail-aware Gaussian belief: correct signal precision and base-rate priors in log space."""
...
@dataclass
class TailAwareGaussianBeliefAgent:
    """
    Like NaiveGaussianBeliefAgent but:

    - Prior means come from ``base_log_levels`` (e.g. ``base_log_levels_from_truth``).
    - Posterior updates weight signals by τ_z = 1 / noise_std² (tail draws carry more precision).
    """

    agent_id: int
    budget: float
    market_ids: tuple[int, ...]
    base_log_levels: dict[int, float]
    ...
    def __post_init__(self) -> None:
        ...
        for m in self.market_ids:
            if m not in self.base_log_levels:
                raise ValueError(f"base_log_levels missing market {m}")
            self._mean_log[m] = float(self.base_log_levels[m])
    ...
    def update_posterior(self, signal: Signal) -> None:
        ...
        if signal.noise_std <= 0:
            return
        obs_prec = 1.0 / (signal.noise_std * signal.noise_std)
...
__all__ = ["TailAwareGaussianBeliefAgent"]
